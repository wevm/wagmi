...
`configureChains` 方法允许使用特定的 provider 来配置你的 chain ，例如：Alchemy、Infura 或其他。这意味着你不需要担心在你的 [connector](/react/connectors/walletConnect#options) 或 [provider](/react/config#provider-optional) 中定义 RPC URLs 和 chain 的配置，这是由 wagmi 内部管理的。
