...
<Callout>
  Not ready to migrate yet? You can find the `0.12.x` docs
...
+const polygonChainId = polygonPos.id
...
+const polygonEtherscanBlockExplorer = polygonPos.blockExplorers.default
...
The `Provider` component no longer supports configuration directly as props. You will now need to create a wagmi [`Client`](/react/config) via `createClient`, and then pass the client to `Provider`:
