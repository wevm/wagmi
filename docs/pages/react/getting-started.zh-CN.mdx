...
<Tabs items={['npm', 'pnpm', 'yarn']}>
  <Tab>```bash npm i wagmi@next viem ```</Tab>
  <Tab>```bash pnpm i wagmi@next viem ```</Tab>
  <Tab>```bash yarn add wagmi@next viem ```</Tab>
</Tabs>
{/* prettier-ignore-end */}
...
接下来，使用 [`createConfig`](/react/config) 创建一个 wagmi `config` 实例，并将 `configureChains` 的结果传递给它。
...
[阅读更多关于 Client 配置](/react/config)。

### 用 `WagmiConfig` Component

最后，用 [`WagmiConfig`](/react/WagmiConfig) Component wrap app，并将 `config` 传递给它。
