...
Force a specific chain id for the request. The wagmi `Client`'s `publicClient` must be set up as a [chain-aware function](/react/publicclient#provider-optional) for this to work correctly.
