...
<Callout>Hook does not run unless `hash` is defined.</Callout>
...
Force a specific chain id for waiting on transaction. The wagmi `Client`'s `publicClient` must be set up as a [chain-aware function](/react/client#publicclient-optional) for this to work correctly.
