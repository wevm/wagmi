...
Next, create a wagmi `config` instance using [`createConfig`](/react/config), and pass the result from `configureChains` to it.
...
Finally, wrap your app with the [`WagmiConfig`](/react/WagmiConfig) component, passing `config` to it.
