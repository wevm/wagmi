...
For instance, in the example provided above `targetQuorum = 2`, however there is only 1 available provider for Avalanche (`staticJsonRpcProvider`), so the quorum will get set to `1`.
