...
Note: Upon connection, the connector will connect to the previously connected chain (or the first chain in `chains`) unless otherwise specified by a [`chainId` config on useConnect](/docs/hooks/useConnect#chainid-optional).
