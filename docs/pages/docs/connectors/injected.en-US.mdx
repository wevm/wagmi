...
`name` can also be set to a function, which has the `detectedName` as the first parameter. `detectedName` can be a list of multiple detected names (e.g. there are multiple injected wallets detected).
