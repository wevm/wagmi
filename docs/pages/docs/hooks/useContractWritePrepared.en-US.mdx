...
Hook for calling an ethers Contract [write](https://docs.ethers.io/v5/api/contract/contract/#Contract--write) method with a ["prepared request"](/TODO). Pairs with the [`usePrepareContractTransaction` hook](/docs/hooks/usePrepareContractTransaction).
...
The prepared request to use when sending the contract transaction. Derived from the [`usePrepareContractTransaction` hook](/docs/hooks/usePrepareContractTransaction).
