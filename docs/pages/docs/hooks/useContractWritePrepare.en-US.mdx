...
```tsx
import { useContractWritePrepare } from 'wagmi'
```

## Usage

`useContractWritePrepare` gives back a "prepared config" to be sent through to `useContractWrite`.

```tsx {4-8}
import { useContractWritePrepare, useContractWrite } from 'wagmi'

function App() {
  const { config, error } = useContractWritePrepare({
