...
```tsx
{
  ...
  connect: (connectorOrConfig: Connector | { connector?: Connector, chainId?: number }) => void
  connectAsync: (connectorOrConfig: Connector | { connector?: Connector, chainId?: number }) => Promise<{
    address: string
    chain: {
      id: number
      unsupported: boolean
