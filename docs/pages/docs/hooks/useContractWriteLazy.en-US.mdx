...
<Callout>You may want to use `useContractWritePrepared` instead. TODO.</Callout>
...
function App() {
  const { data, isError, isLoading, write } = useContractWriteLazy({
...
function App() {
  const contractWrite = useContractWriteLazy({
    addressOrName: '0xecb504d39723b0be0e3a9aa33d646642d1051ee1',
    contractInterface: wagmigotchiABI,
    functionName: 'feed',
  })
