...
Hook for declaratively waiting until transaction is processed. Pairs well with [`useContractWriteLazy`](/docs/hooks/useContractWriteLazy)/[`useContractWritePrepared`](/docs/hooks/useContractWritePrepared) and [`useSendTransaction`](/docs/hooks/useSendTransaction).
...
Transaction hash to monitor. Works well with [`useContractWriteLazy`](/docs/hooks/useContractWriteLazy)/[`useContractWritePrepared`](/docs/hooks/useContractWritePrepared) and [`useSendTransaction`](/docs/hooks/useSendTransaction) `data.hash`.

```tsx {5}
import { useContractWritePrepared, useWaitForTransaction } from 'wagmi'

function App() {
  const contractWrite = useContractWritePrepared({
...
Function that resolves to processed transaction receipt. Works well with [`useContractWriteLazy`](/docs/hooks/useContractWriteLazy)/[`useContractWritePrepared`](/docs/hooks/useContractWritePrepared) and [`useSendTransaction`](/docs/hooks/useSendTransaction) `data.wait`.
