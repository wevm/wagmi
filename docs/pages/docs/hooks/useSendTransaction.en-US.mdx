...
- `dangerouslyUnprepared`: Allow to pass through an adhoc unprepared `request`. Note: This has harmful UX side-effects, it is highly recommended to not use this and instead prepare the request upfront using the `usePrepareSendTransaction` hook.
