...
<Callout>
  ...
  [`usePrepareSendTransaction` hook](/docs/hooks/usePrepareSendTransaction) to
...
- `dangerouslyUnprepared`: Allow to pass through an adhoc unprepared `request`. Note: This has [UX pitfalls](/docs/prepare-hooks#ux-pitfalls-without-prepare-hooks), it is highly recommended to not use this and instead prepare the request upfront using the `usePrepareSendTransaction` hook.
...
> This is automatically populated when using [`usePrepareSendTransaction` hook](/docs/hooks/usePrepareSendTransaction).
