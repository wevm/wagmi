...
Contract address or ENS name. If `addressOrName` is not defined, response is `null`.
...
### contractInterface (optional)

Contract ABI. If `contractInterface` is not defined, response is `null`.
