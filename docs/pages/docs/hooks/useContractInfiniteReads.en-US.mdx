...
Enables smart replcaing of cached data between queries. If some subtree of new value equals to old, hook will reuse it to save link and prevent rerenders. Defaults to `true`.
...
**Deprecated**. Use `structuralSharing` instead.

Determines if the data has changed or not. Defaults to `undefined`.
