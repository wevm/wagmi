...
## Usage useWaitForTransaction() with useContractWrite()

```tsx
import { useContractWrite, useWaitForTransaction } from 'wagmi'

const App = () => {
  const [callFeedData, write: callFeed] = useContractWrite(
    {
  ...
  const [{ data, error, loading }, wait: waitForFeedTransaction] = useWaitForTransaction({
    skip: true,
  })

  const someActionHandler = async () => {
       const txn = await callFeed();
       await waitForFeedTransaction({wait: txn.data.wait});
  }
