...
Waits until _confirmations_ number of blocks are mined on top of block transaction was mined. Defaults to `1`. If _confirmations_ is `0`, hook will not wait and return immediately without blocking, likely resulting in `data` being `null`.
...
Hash of transaction to wait for.
...
Function that resolves to processed transaction receipt. Works well with [**useTransaction**](/docs/hooks/useTransaction) result.
