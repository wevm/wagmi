...
Performing asynchronous tasks in an event handler (like a button click) or in a non-initiated user event (like a page load) is not ideal as it can lead to a variety of connected UX pitfalls.

Prepare hooks solve these pitfalls as they allow you to eagerly perform this asynchronous work beforehand & generate the parameters required for their paired hook (e.g. `usePrepareSendTransaction` prepares a request for `useSendTransaction`).
