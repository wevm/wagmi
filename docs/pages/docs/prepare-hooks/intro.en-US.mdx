...
Performing asynchronous tasks in an event handler (like a button) or in a non-initiated user event (like a page load) is not ideal as it can lead to a variety of connected user-experience (UX) related pitfalls ([listed below](#ux-pitfalls-without-prepare-hooks)).
