...
const { chains, provider } = configureChains(defaultChains, [
  alchemyProvider(process.env.ALCHEMY_ID),
  fallbackProvider(),
])
...
<Callout emoji="🔗">
  The `fallbackProvider` ensures that your chains always have an RPC URL to fall
...
const { chains } = configureChains(
  [chain.mainnet, chain.optimism],
  [alchemyProvider(process.env.ALCHEMY_ID), fallbackProvider()],
)
```
