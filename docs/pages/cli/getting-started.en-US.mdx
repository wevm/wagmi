...
Run the `init` command to generate a configuration file: either `wagmi.config.ts` if TypeScript is detected, otherwise `wagmi.config.js`. You can also create the configuration file manually. See [Configuration](/cli/configuration) for more info.
...
import { useEnsRegistryContract, useErc20BalanceOf, useErc20Read } from './generated'

// Use the generated ENS Registry ABI and address variables
