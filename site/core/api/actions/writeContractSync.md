...
Action for executing a write function on a contract synchronously. Waits for the transaction to be included in a block before returning.
...
const result = await writeContractSync(config, {
  abi,
...
Pairing [`simulateContract`](/core/api/actions/simulateContract) with `writeContractSync` allows you to validate if the transaction will succeed ahead of time. If the simulate succeeds, `writeContractSync` can execute the transaction.

::: code-group
```ts [index.ts]
import { simulateContract, writeContractSync } from '@wagmi/core'
import { abi } from './abi'
import { config } from './config'
...
const result = await writeContractSync(config, request)
```
<<< @/snippets/abi-write.ts[abi.ts]
<<< @/snippets/core/config.ts[config.ts]
:::
...
```ts
import { type WriteContractSyncParameters } from '@wagmi/core'
```
...
::: code-group
```ts [index.ts]
import { getConnection, writeContractSync } from '@wagmi/core'
import { abi } from './abi'
import { config } from './config'
...
```ts
import { type WriteContractSyncReturnType } from '@wagmi/core'
```
...
- [`writeContractSync`](https://viem.sh/docs/contract/writeContractSync)
