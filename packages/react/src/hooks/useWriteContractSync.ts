...
> = UseMutationReturnType<
  ...
  context
> & {
  mutate: WriteContractSyncMutate<config, context>
  mutateAsync: WriteContractSyncMutateAsync<config, context>
...
>(
  ...
  const mutationOptions = writeContractSyncMutationOptions(config)
