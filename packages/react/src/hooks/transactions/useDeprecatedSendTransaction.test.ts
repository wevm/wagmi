...
import {
  ...
  UseDeprecatedSendTransactionConfig,
  useDeprecatedSendTransaction,
} from './useDeprecatedSendTransaction'

function useDeprecatedSendTransactionWithConnect(
  config?: UseDeprecatedSendTransactionArgs &
    UseDeprecatedSendTransactionConfig,
) {
  return {
    connect: useConnect(),
    sendTransaction: useDeprecatedSendTransaction(config),
  }
}

describe('useDeprecatedSendTransaction', () => {
  it('mounts', () => {
    const { result } = renderHook(() => useDeprecatedSendTransaction())
    ...
    describe('chainId', () => {
      it('switches before sending transaction,', async () => {
        const utils = renderHook(() =>
          useDeprecatedSendTransactionWithConnect({
            chainId: 1,
            request: {
