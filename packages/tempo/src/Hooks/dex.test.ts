...
describe('useBuy', () => {
  test('default', async () => {
    ...
    await dexActions.placeSync(config, {
      token: base,
...
describe('useCancel', () => {
  test('default', async () => {
    ...
    const dexBalanceBefore = await dexActions.getBalance(config, {
    ...
    const dexBalanceAfter = await dexActions.getBalance(config, {
      account: account.address,
      token: quote,
    })
...
describe('useCreatePair', () => {
  test('default', async () => {
    ...
    const { token: baseToken } = await tokenActions.createSync(config, {
...
describe('useBalance', () => {
  test('default', async () => {
    ...
    const { orderId } = await dexActions.placeSync(config, {
      token: base,
    ...
    await dexActions.cancelSync(config, {
      orderId,
    })
  ...
  test('behavior: balances are per-token', async () => {
    ...
    await dexActions.cancelSync(config, { orderId })
...
describe('useOrder', () => {
  ...
  test('behavior: returns flip order details', async () => {
    ...
    // Place a flip order
    const { orderId } = await dexActions.placeFlipSync(config, {
      token: base,
      amount: parseUnits('50', 6),
      type: 'buy',
      tick: Tick.fromPrice('1.001'),
...
describe('useWatchOrderCancelled', () => {
  ...
  test('behavior: filter by orderId', async () => {
    ...
    // Place two orders
    const { orderId: orderId1 } = await dexActions.placeSync(config, {
      token: base,
      amount: parseUnits('100', 6),
      type: 'buy',
      tick: Tick.fromPrice('1.001'),
    ...
    const { orderId: orderId2 } = await dexActions.placeSync(config, {
      token: base,
      amount: parseUnits('50', 6),
      type: 'buy',
      tick: Tick.fromPrice('1.001'),
...
describe('useWithdraw', () => {
  test('default', async () => {
    ...
    // Get DEX balance
    const dexBalance = await dexActions.getBalance(config, {
      account: account.address,
      token: quote,
    })
    ...
    const walletBalanceBefore = await tokenActions.getBalance(config, {
      token: quote,
      account: account.address,
    })
    ...
    const walletBalanceAfter = await tokenActions.getBalance(config, {
      token: quote,
      account: account.address,
    })
